...
<script>
  export default {
    ...
    methods: {
      ...
      // 设置置顶
      setToTop(row) {
        this.$postJson(this._basePath.ajaxSingleGoodSetTop, {id: row.id, singleSetTop: true}).then(res => {
          if (res.success) {
        ...
        if (isCancel) {
          this.$confirm('确认取消推荐吗？', '提示', {
            ...
            .then(() => {
              ...
              this.$postJson(this._basePath.ajaxRecommendSingleGood, para).then(res => {
                if (res.success) {
                  this.$message({
                    message: '取消成功',
                    type: 'success'
      ...
      getUsers() {
        const para = {
          ...
          isSingle: this.switchStatus !== '0'
        }
        this.loading = true
        this.$postJson(this._basePath.ajaxSingleGoodByPage, {...para, ...this.filters}).then(res => {
          this.loading = false
          if (res.success) {
