  ...
  <section class="app-container" id="hang_bill">
    ...
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
      ...
      <el-table :data="users" @selection-change="selsChange" highlight-current-row v-loading="loading" style="width: 100%;">
        ...
        <template v-if="switchStatus === '0'">  
          <el-table-column label="商品浏览量" prop="browseCount">
          </el-table-column>
        </template>
        <template v-else>
...
<script>
  export default {
    ...
    methods: {
      ...
      // 设置置顶
      setToTop(row) {
        this.$postJson(this._basePath.ajaxSetSuperiorGoodToTop, {id: row.id, boutiqueIsSetTop: true}).then(res => {
          if (res.success) {
          ...
          this.$confirm('确认取消推荐吗？', '提示', {
            ...
            .then(() => {
              const para = { 
                ids: ids.toString(),
                isBoutique: false,
                boutiqueIsSetTop: false
              }
              this.$postJson(this._basePath.ajaxSetSuperiorGoodToShow, para).then(res => {
                if (res.success) {
                  this.$message({
                    message: '取消成功',
                    type: 'success'
      ...
      getUsers() {
        const para = {
          ...
          isBoutique: this.switchStatus !== '0'
        }
        this.loading = true
        this.$postJson(this._basePath.ajaxAllFilterGood, {...para, ...this.filters}).then(res => {
          this.loading = false
          if (res.success) {
